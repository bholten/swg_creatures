...
These are the spawn zones where this creature's lairs can appear, with geographic coordinates.
...
spawn_locations <- lair_spawn_groups %>%
  ...
  left_join(zones, by = c("spawnGroupName" = "spawnZone")) %>%
  select(planet, spawnGroupName, lairTemplateName, zoneType,
         x, y, x2, y2, r,
         minDifficulty, maxDifficulty, numberToSpawn, weighting, size)
...
creature_static <- static_spawns %>%
  ...
  select(planet = zone_id, x, y, z, respawn_time, heading = header, parent_id)
